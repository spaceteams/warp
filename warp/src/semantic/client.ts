...
/**
 * Creates a client component factory that bundles multiple method factories into
 ...
 *   are forwarded to every method invocation. `kind` is automatically set to "client".
 * @param factories - Record of named method factories.
 *
 * @example
 * ```ts
 * const apiClient = client({ name: "api-client" }, {
 *   getUser: (ctx) => async (id: string) => ctx.fetch(`/users/${id}`),
 * });
 * ```
 */
export function client<const T extends Record<string, unknown>, RunOptions = unknown>(
