...
/**
 * Creates a repository component factory that bundles multiple method factories into
 ...
 *   are forwarded to every method invocation. `kind` is automatically set to "repo".
 * @param factories - Record of named method factories.
 *
 * @example
 * ```ts
 * const userRepo = repo({ name: "user-repo" }, {
 *   findById: (ctx) => async (id: string) => ctx.db.find(id),
 *   create: (ctx) => async (data: UserData) => ctx.db.insert(data),
 * });
 * ```
 */
export function repo<const T extends Record<string, unknown>, RunOptions = unknown>(
